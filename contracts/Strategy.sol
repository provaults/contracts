...
contract Strategy is StratManager, FeeManager {
    ...
    constructor(
        ...
        address _keeper,
        address _strategist,
        address _proVaultsFeeRecipient,
        address[] memory _outputToNativeRoute,
        address[] memory _outputToLp0Route,
        address[] memory _outputToLp1Route
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _proVaultsFeeRecipient) public {
        want = _want;
        poolId = _poolId;
        chef = _chef;
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IMasterChef(chef).deposit(wantBal, poolId);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMasterChef(chef).withdraw(_amount.sub(wantBal), poolId);
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IMasterChef(chef).deposit(0, poolId);
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 proVaultsFeeAmount = nativeBal.mul(proVaultsFee).div(MAX_FEE);
        IERC20(native).safeTransfer(proVaultsFeeRecipient, proVaultsFeeAmount);
    ...
    function rewardsAvailable() public view returns (uint256) {
        return IMasterChef(chef).pendingReward(address(this), poolId);
    }
